...
@Component({selector: 'app-add-order', templateUrl: './add-order.component.html', styleUrls: ['./add-order.component.scss']})
export class AddOrderComponent implements OnInit {
  ...
  constructor(private formBuilder : FormBuilder, private db : AngularFirestore, private router : Router, private ps : ProductService) {
    const state = this
      .router
  ...
  private initForm() {
    this.orderForm = this
      ...
      .group({
        name: [
          this.order.userDetails.displayName,
          [Validators.required]
        ],
        email: [
          this.order.userDetails.email,
          [Validators.required]
        ],
        phone: [
          this.order.userDetails.phoneNumber,
          [Validators.required]
        ],
        address: [
          this.order.userDetails.address,
          [Validators.required]
        ],
        zone: [
          this.order.userDetails.zone,
          [Validators.required]
        ],
        locality: [this.order.userDetails.locality, []
        ],
        dateOfDelivery: [
          (this.order.deliveryDate
            ? (this.order.deliveryDate as firebase.firestore.Timestamp).toDate()
            : null),
          [Validators.required]
        ]
      });
  ...
  submitForm() {
    if (!this.orderForm.valid) {
      ...
      let curOrder : Order = {
        ...
        createdAt: this.order.createdAt || firebase
          .firestore
          .FieldValue
          .serverTimestamp(),
        updatedAt: null,
        deliveryDate: o.dateOfDelivery,
        isManual: true
      };
      ...
      if (this.mode == MODE.new) {
        ...
        this
          .order
          .cart
          .forEach((d) => (idMap[d.id] = d));
        curOrder.cart = curOrder
          .cart
          .map((item) => {
            if (idMap[item.id]) {
              return {
  ...
  private getDeliveryDate() {
    ...
    // 'Saturday']; Thur 9PM to Mon 9PM - message should show “delivery to be done
    // on Wednesday” otherwise “Saturday”
    ...
    if ((curDay == 4 && hour >= 21) || curDay > 4 || (curDay == 1 && hour < 21) || curDay < 1) {
      dayString = 'Wednesday';
      ...
      // get the second delivery date days count cound second delivery date by adding
      // curDay to wednesday(3)
  ...
  getDateStringFromDate(date, daysTillNextDeliveryDate) {
    ...
    // get month after the updated date (above setDate), so that updated month (in
    // case adding days changes month) is fetched
    const curMonth = date.getMonth();
    const deliveryDate = date.getDate();
    let dateStr = (deliveryDate > 9
      ? deliveryDate
      : '0' + deliveryDate) + '/' + (curMonth > 8
      ? curMonth + 1
      : '0' + (curMonth + 1)) + '/' + date.getFullYear();
    return {date, dateStr};
  }
